...
resource "azurerm_application_insights" "AL" {
  name                = "${var.class_name}${var.student_name}${var.environment}${random_integer.deployment_id_suffix.result}in"
  location            = azurerm_resource_group.rg.location
  resource_group_name = azurerm_resource_group.rg.name
...
resource "azurerm_app_service_plan" "fp" {
...
resource "azurerm_function_app" "fap" {
